...
public sealed class FfmpegService
{
    ...
    public async Task<FfmpegResult<StreamAnalysisResult>> AnalyzeAudioStream(FileInfo file)
    {
        ...
        Debug.Assert(!string.IsNullOrWhiteSpace(returnData.Streams[0].Channels));
    ...
    {
        string outputFile = Path.Combine(_outputDir, "TMech_Stripped_" + file.Name);
    ...
    public async Task<FfmpegResult<FileInfo>> NormalizeLoudness(FileInfo file, StreamAnalysisResult streamData, LoudnormResult loudnessData)
    {
        ...
        string outputFile = Path.Combine(_outputDir, "TMech_Normalized_" + file.Name);
        var returnData = new FileInfo(outputFile);
        ...
        if (!returnData.Exists)
        {
            ...
            {
                Success = false,
                Output = "Error applying loudness normalization: no output file was generated by ffmpeg indicating that something went wrong"
                + "PROCESS OUTPUT:"
                + string.Join(System.Environment.NewLine, output)
            };
        }
